...
class CLI_Command extends WP_CLI_Command {
     ...
     *
    * [--create-missing=<0|1>] (DEPRECATED)
    * [--apply-variations=<0|1>] (DEPRECATED)
    * [--dry-run=<0|1>] Simula sem aplicar.
    * [--hydrate-variations=<0|1>] (DEPRECATED)
    * [--aggressive-hydrate-variations=<0|1>] (DEPRECATED)
     */
    public function map( array $args, array $assoc_args ): void {
        ...
        $deprecated_flags = [];
        foreach ( [ 'create-missing', 'apply-variations', 'hydrate-variations', 'aggressive-hydrate-variations' ] as $flag ) {
            if ( isset( $assoc_args[ $flag ] ) ) { $deprecated_flags[] = $flag; }
        }
        if ( $deprecated_flags ) {
            \WP_CLI::warning( 'Flags depreciadas ignoradas: ' . implode( ', ', $deprecated_flags ) . ' (>=0.3.0 comportamento é determinístico: cria apenas termos marcados e sempre atualiza variações).' );
        }
        $term_config = [];
        foreach ( $term_inputs as $input ) {
            ...
            $term_config[] = [
                ...
                'term_slug'   => $slug,
                'create'      => true, // CLI assume criação explícita para cada termo informado
            ];
        }
        ...
        $options = []; // legacy options removidas

        foreach ( $products as $product_id ) {
            $corr_id = uniqid( 'l2g_cli_', true );

            if ( ! empty( $assoc_args['dry-run'] ) ) {
                $result = $this->mapping->dry_run( (int) $product_id, $mapping, [], $corr_id );
                ...
                $result = $this->mapping->apply( (int) $product_id, $mapping, [], $corr_id );
     ...
     *
    * (Flags legacy --hydrate / --aggressive serão ignoradas se passadas.)
     */
    public function variations_update( array $args, array $assoc_args ): void {
        ...
        if ( isset( $assoc_args['hydrate'] ) || isset( $assoc_args['aggressive'] ) ) {
            \WP_CLI::warning( 'Flags --hydrate / --aggressive depreciadas e ignoradas.' );
        }
